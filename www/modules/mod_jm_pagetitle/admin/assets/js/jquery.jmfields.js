...
(function ($) {
	...
	}		$.fn.jmMedia= function(){		this.each(function () {			var $this = $(this);			$this.parent().parent().css({position:'relative'});			$this.parent().parent().find('.modal').addClass('media-upload-btn');			$this.parent().parent().find('a').not('.modal').addClass('jmmedia-clear').html('X');		});	}
	$(document).ready(function(){
		$('.jm-field input[type=radio]').jmRadio();
		$('select.jm-field.multi').jmSelectMulti();				$('input.jm-field.jmmedia').jmMedia();
		$('select.jm-field.single,#jform_params_layout').jmSelectSingle();
