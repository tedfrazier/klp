...
class JFormFieldLayout extends JFormField {
	protected $type = 'Layout'; //the form field type
	    ...
	    $i=0;
	    	...
	    	if(is_dir(JPATH_SITE . '/templates/' . $this->getTemplate() . '/html/mod_jmparallax/'))
	    	if ($handle = opendir(JPATH_SITE . '/templates/' . $this->getTemplate() . '/html/mod_jmparallax/')) {
		    while (false !== ($entry = readdir($handle))) {
		    	...
		    	if ($entry != "." && $entry != ".." &&$entry!="index.html"){
		    		if($this->value==str_replace('.php','',$entry)) {
						...
						$order = 9999;
						ob_start();
						readfile(JPATH_SITE . '/templates/' . $this->getTemplate() . '/html/mod_jmparallax/' . $entry);
		...
		if ($handle = opendir(JPATH_SITE.'/modules/mod_jmparallax/tmpl/')) {
		    while (false !== ($entry = readdir($handle))) {
		    	...
		    	if ($entry != "." && $entry != ".." &&$entry!="index.html"){
		    		if($this->value==str_replace('.php','',$entry)) {
						...
						$order = 9999;
						ob_start();
						readfile(JPATH_SITE.'/modules/mod_jmparallax/tmpl/' . $entry);
						$file_content = ob_get_clean();
						preg_match("'<!--layout:(.*?),order:(.*?)-->'si", $file_content, $match);
						if(isset($match[1])) $layout_name = $match[1];
						if(isset($match[2])) $order = $match[2];
