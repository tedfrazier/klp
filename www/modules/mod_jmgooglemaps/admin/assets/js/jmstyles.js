...
var JM=JM||{};
(function(){
    JM.StyleList = new Class({
        Implements:[Options,Events],
        options:{},
        initialize:function(options){
            
            this.setOptions(options);
            var styles =JSON.decode(new JM.Base64().base64Decode(this.options.encodedItems));
            var self=this;
            ...
            document.adminForm.onsubmit=function(){
                var styles=[];
                var i=0;
                $$("#jmg-styles-container li").each(function(li){
                ...
                $("jmg-style-hidden").set("value",new JM.Base64().base64Encode(JSON.encode(styles)));
                if(submit!=null){
                    submit()
                }
            }
        ...
        openDialog: function(edit, li, style){
           ...
           if(jQ("#jmg-style-dialog").length >0){
        	   jQ('#jmg-style-dialog').remove();
           }
            jQ("#style-form").append(dialog);
            
            // this code will setup show/ hide form, bind event when create/update style
            jQ('#jmg-feature-type, #jmg-style-element-type,#jmg-style-visibility').chosen();
            // call color picker
            jQ('.jm_color').ColorPicker({
        		color: '#0000ff',
            ...
            if(!edit){
                ...
                jQ(self.options.btnCreateID).unbind('click').bind('click', function(){
                    jQ('#jmg-style-messages').html('');
                
                //create style
                var msg = '';
                var style = new Object();
                style.featureType 		= jQ('#jmg-feature-type').val();
                style.elementType 		= jQ('#jmg-style-element-type').val();
                style.visibility		= jQ('#jmg-style-visibility').val();
                style.invertLightness 	= jQ('#jmg-style-invert-lightness').val();
                style.mapColor 			= jQ('#jmg-style-map-color').val();
                style.weight 			= jQ('#jmg-style-weight').val();
                style.hue 				= jQ('#jmg-style-hue').val();
                style.saturation 		= jQ('#jmg-style-saturation').val();
                style.lightness 		= jQ('#jmg-style-lightness').val();
                style.gamma 			= jQ('#jmg-style-gamma').val();
                ...
                */
                ...
                self.add(style);
                //khoi tao lai form
                jQ('#jmg-style-messages').html('').append(jQ('<div>').addClass('jm-message success').html(self.options.warningText.addStyleSuccess));
                ...
                jQ('#jmg-feature-type').val(style.featureType).trigger('liszt:updated');
                jQ('#jmg-style-element-type').val(style.elementType).trigger('liszt:updated');
                jQ('#jmg-style-visibility').val(style.visibility).trigger('liszt:updated');
                jQ('#jmg-style-invert-lightness').val(style.invertLightness);
                jQ('#jmg-style-map-color').val(style.mapColor);
                jQ('#jmg-style-weight').val(style.weight);
                jQ('#jmg-style-hue').val(style.hue);
                jQ('#jmg-style-saturation').val(style.saturation);
                jQ('#jmg-style-lightness').val(style.lightness);
                jQ('#jmg-style-gamma').val(style.gamma);
            ...
            $$('#jmg-form-style .jm_switch').each(function(el)
            {
        ...
        update: function(li, style){
            ...
            var featureType 		= jQ('#jmg-feature-type').val();
            var elementType 		= jQ('#jmg-style-element-type').val();
            var visibility			= jQ('#jmg-style-visibility').val();
            var invertLightness 	= jQ('#jmg-style-invert-lightness').val();
            var mapColor 			= jQ('#jmg-style-map-color').val();
            var weight 				= jQ('#jmg-style-weight').val();
            var hue 				= jQ('#jmg-style-hue').val();
            var saturation 			= jQ('#jmg-style-saturation').val();
            var lightness 			= jQ('#jmg-style-lightness').val();
            var gamma 				= jQ('#jmg-style-gamma').val();
            ...
            jQ('#jmg-style-messages').html('').append(jQ('<div>').addClass('jm-message success').html(this.options.warningText.updateMarkerSuccess));
        ...
        removeAll: function(){
            if(confirm(this.options.warningText.confirmDeleteAll)){
                ...
                jQ('#jmg-styles-container').show();
                /* this.showMessage(this.options.warningText.deleteAllSuccess); */
                setTimeout(function(){
                    jQ('#jmg-styles-container').slideUp(500)
                }, 1500);
            }
            return false;
        },
