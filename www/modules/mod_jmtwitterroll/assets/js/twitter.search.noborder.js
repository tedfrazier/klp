...
(function($) {
	...
	$.fn.twitterSearchesN.formatter = function(json, opts) {
		...
		pretty = prettyDate(json.created_at);
		 str += '<div class="twitterSearchesNText">' + text + '</div>'
		 if (opts.time && pretty)
			str += ' <span class="twitterSearchesNTime">('+ pretty +')</span><span class="twitterSearchesNUser"><a href="http://www.twitter.com/'+ json.user.name+'"/ TARGET="_blank">Follow Us - @' 
		  + json.user.name + '</a></span></div></div>';
		 return str;
	};
	...
	$.fn.twitterSearchesN.defaults = {
		...
		css: {
			...
			frame: {  },
			tweet: { padding: '15px 0px', clear: 'left' },
			img:   { 'float': 'left', margin: '5px', width: '30px', height: '21px' },
			loading: { padding: '20px', textAlign: 'center', color: '#888' },
			text:  {},
			time:  { fontSize: '13px', color: '#888', lineHeight: '1em', marginBottom: '10px' },
			user:  { fontWeight: 'normal', fontSize: '14px', fontFamily: 'open_sansbold' }
		}
	};
