...
<form id="finder-search" action="<?php echo JRoute::_($this->query->toURI()); ?>" method="get">
	...
	<fieldset class="word row-fluid">
		<label for="q" class="span2">
			<?php echo JText::_('COM_FINDER_SEARCH_TERMS'); ?>
		</label>
		<div class="span4">
			<input type="text" name="q" id="q" size="30" value="<?php echo $this->escape($this->query->input); ?>" class="inputbox <?php echo (Helix::direction()=='rtl')?'right':'left'; ?>" />
		</div>
		<div class="span6">
			<?php if ($this->escape($this->query->input) != '' || $this->params->get('allow_empty_search')):?>
				<button name="Search" type="submit" class="btn btn-default btn-reamode <?php echo (Helix::direction()=='rtl')?'right':'left'; ?>"><?php echo JText::_('JSEARCH_FILTER_SUBMIT');?></button>
			<?php else: ?>
				<button name="Search" type="" class="btn btn-default btn-readmore <?php echo (Helix::direction()=='rtl')?'right':'left'; ?>"><?php echo JText::_('JSEARCH_FILTER_SUBMIT');?></button>
			<?php endif; ?>
		</div>
	</fieldset>
